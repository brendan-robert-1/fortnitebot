...
async def kdr(ctx, user, scope, game_mode):
    if(not check_scope(scope)):
        ...
        rank = str(kdr_stats['rank'])
        scopeStr = get_scope_str(scope)
        msg = user + " has a K/D ratio of " + value + " for " + game_mode+ scopeStr + scope + ". This puts " + user + " in the top " + percentile + " percentile, at rank " + rank + "." 
        await bot.say(msg)
...
async def trn(ctx, user, scope, game_mode):
    if(not check_scope(scope)):
        ...
        rank = str(trn_stats['rank'])
        scopeStr = get_scope_str(scope)
        msg = user + " has a TRN Rating of " + value + " for " + game_mode + scopeStr + scope + ". This puts " + user + " in the top " + percentile + " percentile, at rank " + rank + "."
        await bot.say(msg)
...
async def winrate(ctx, user, scope, game_mode):
    if(not check_scope(scope)):
        ...
        rank = str(win_stats['rank'])
        scopeStr = get_scope_str(scope)
        msg = user + " has a win ratio of " + value + "% for " + game_mode + scopeStr + scope + ". This puts " + user + " in the top " + percentile + " percentile, at rank " + rank + "."
        await bot.say(msg)
